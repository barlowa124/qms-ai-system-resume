...
### A prospective risk analysis of AI-assisted deviation report review, and an instrument for assessing an already-built AI system
...
<!--
Speaker notes: Open with who you are before what you found: a computer-science-background student in this program, and this placement was your first exposure to the life sciences industry. That framing matters twice over. It explains why you noticed what an insider might not, and it frames this talk as a statement of how you intend to practice rather than a grievance against a former employer. Then set scope honestly: this is not an argument that AI has no place in quality systems, but an argument that deviation management is an unusually bad place to start, and that the assessment tooling the industry has does not yet ask the right questions. Say up front that no employer or client is named, and that nothing here is a compliance determination about any specific system.
-->
...
- The tool never reached deployment, so this is a prospective risk analysis: it does not describe a live failure.
...
<!--
Speaker notes: Orient the case study before any findings: one tool, one placement, never deployed. Present it as a case study submitted for review, and let any implication about their curriculum surface later from the findings themselves rather than asserting it now. Every AI-assisted quality system is justified on the basis that a qualified human reviews the output, so the entire safety argument depends on the reviewer's competence. Frame that as an observation from the case study rather than a claim about this program. Do not rush this slide.
-->
...
- A tool that reviews draft reports before QA reaches them shapes the record the decision is made from; it never makes the disposition decision itself.
...
> Influencing the quality of the record falls short of deciding the outcome, yet it still carries real weight. Hold both of those at once.

<!--
Speaker notes: Be precise here, because the whole talk depends on it and it is the easiest thing to overstate: a draft-review aid determines which weaknesses get fixed before a qualified reviewer ever sees the report, without classifying or dispositioning anything. If someone objects that this makes the tool low-risk, agree that it lowers the ceiling on the risk and then ask how you would know if it started being used as a readiness gate instead of a drafting aid. Name the regulatory argument explicitly on this slide, because it is the premise every later finding tests rather than contradicts. 'Optional, with a human always approving' is the whole compliance case in one sentence, and it is true as designed. Do not resolve the tension yet; just mark that it exists, and that the human-oversight finding later in the deck is a direct test of whether 'optional' survives contact with a 99 percent acceptance rate.
-->
...
- High volume and text-heavy, with the same handful of investigation patterns recurring case after case: the exact profile language models handle well.
...
- Those targets are achievable. A tool that hits them can still be unsafe without anyone noticing.
- In a meeting, my supervisor told me directly that a single failed batch costs on the order of $9 million.
- At that price point, a tool that reliably caught what leads to a failed batch would have value well beyond one site's efficiency metrics: the kind of value that could plausibly be sold to the wider life sciences industry as its own product rather than used only internally.
...
<!--
Speaker notes: Resist the temptation to strawman the business case. If the audience thinks you are hostile to the technology, credibility is gone. The efficiency gains are real, the backlog pressure is real, and the people driving these programs are usually competent and well-intentioned. The problem is not motive. Be precise about provenance on the last two bullets. The $9 million figure is a number one supervisor stated verbally in one meeting; it is not an audited or published company figure, so say so if asked. The commercial-product point is your own inference about incentive; nobody told you it was the plan. Both bullets exist only to explain the scale of the pressure driving speed, not to imply anyone had a hidden commercial motive; keep that distinction explicit if challenged.
-->
...
- The engineering discipline in this organization demonstrably existed; it simply had not reached the AI layer.
- The gap reflected capacity, not indifference: IT was already stretched sustaining its existing stack, and the time of both the executive IT sponsor and the senior engineers who could have closed the gap was consistently occupied by other priorities.
...
- Senior engineering capacity existed (developers with a decade or more of experience) and was assigned to the commercial product and the data platform instead of the work entering the deviation workflow.
...
- I am the evidence for this one: not that the judgment was wrong, but that no step existed that could have established it either way.
...
- There was no procedure governing any of it: no acceptable-use rule, and no data-classification rule for prompts. Nothing even required AI-assisted work to be recorded at all.
...
- That same volunteered-interest question is also where the build side shows up: interest from the IT function specifically was effectively negligible on it, on a question that cannot separate genuine disinterest from a team that already had no spare bandwidth to take on more. Consistent with that figure, not one person from that function reached out to me about the project directly at any point during my time there.
- Job impact and environmental cost dominated the free-text responses, alongside a broader worry about whether it would help daily work at all; how to supervise a model was not one of them.
...
- In the entire time I was there, not one person asked me whether it was safe. Nobody asked whether its output could affect a decision or whether it belonged that close to a GMP process, and nobody raised the possibility of accidental prompt injection or accidental drift.

> Wanting a tool badly says nothing about being ready to supervise it. A population that cannot describe how it behaves cannot be the effective control on it.

<!--
Speaker notes: This is the second readiness gap and it pairs with the engineering one: the layer was under-supported technically, and the population designated to supervise its output had little working familiarity with model behavior. The figures are the organization's own, from its own survey, which is what makes them usable. You are not characterizing colleagues from impression, you are reporting what the organization measured about itself. Keep the third bullet in reserve for the discussion, because it is the one that closes the argument. Nearly half the workforce named unclear rules as a barrier and asked for guidance above all else, which means the missing procedure was not only a governance defect visible to auditors: it was the single thing staff most wanted and did not have. The survey's own executive summary concluded that the constraint was trust and governance rather than lack of interest. If challenged on whether that is your interpretation, say that it was the organization's. The IT-interest bullet deliberately gives no count and no denominator. State it only as 'effectively negligible'. A specific number on a small function is exactly the kind of figure that narrows down who did or did not volunteer, which this deck avoids everywhere else and must avoid here too. If asked whether this contradicts the capacity framing on the asymmetry slide, do not deny the tension; name it honestly: a survey question about volunteering for more work cannot cleanly separate genuine disinterest from a team that already had no spare bandwidth, and this data cannot resolve that ambiguity by itself. What you can say is that IT was independently documented elsewhere in this case study as carrying the heaviest existing workload of any function measured, which is at least consistent with capacity, though it does not prove capacity was the only factor. The free-text responses included a minority of strongly hostile comments. Do not quote them and do not characterize their authors. The substantive concerns inside them, environmental cost and job displacement, are already on the slide and are the part that deserves an answer. Give the organization full credit for the survey response: it is evidence of genuine appetite, and saying so keeps this from sounding dismissive. The gap separates appetite from preparation; it says nothing about who is clever and who is foolish. These were experienced colleagues who engaged willingly and spoke candidly, and the enthusiast population is the fairest sample you could have drawn: if familiarity is thin there, it is thinner everywhere else. The temperature question is a fair probe rather than a trivia test, because sampling behavior is what determines whether the same input yields the same output twice, and that is the first thing a reviewer needs to know. If challenged that this is just normal early adoption, agree: appetite outrunning familiarity is ordinary and harmless everywhere except where the unfamiliar population is the designated control. Name nobody and quote nobody. Deliver the training-program bullet as a fact about organizational sequencing rather than a credential. What matters is not that you were asked, but that even the organization's own remedy was scoped to arrive during or after the thing it was meant to prepare people for. That is the timing failure in one sentence, and it needs no further comment from you. The final bullet is the concrete counterpart to everything the survey measured in the abstract. It is a first-hand observation about you rather than a claim about anyone else's competence, so it carries none of the naming risk the rest of this slide is careful about. Deliver it as absence of evidence you personally looked for throughout your time there, rather than a survey of what colleagues knew: you cannot know what every individual silently understood, only that in your own experience the question never once came up unprompted. Prompt injection and drift are named here because they are the two failure modes least likely to announce themselves, which is exactly why nobody asking about them is the finding rather than a reassuring silence.
-->
...
- The tool never reached deployment, so what follows is the pattern this class of system follows elsewhere, not an account of what happened here.
...
- The parallel to this case study is structural rather than coincidental: in both examples, the metric the organization could see (vendor-reported accuracy, adoption scale) was not the metric that mattered, and no independent validation requirement stood between the two.
...
- Human review is documented and enforced in the workflow. It shows up in every audit trail.
...
- On the ground, open caseload and case-age were not spread evenly across the reviewing team: a small subset of reviewers routinely carried disproportionate queue depth and the oldest unresolved records, exactly the variable the diagnostic signature depends on segmenting by; an aggregate acceptance rate would never surface it.
- In conversation, the QA and investigation team named that workload directly as a driver of turnover, citing more than one resignation during my own time on the placement.
- This is documented within life sciences rather than imported from another industry: reviews of computerized drug-safety alerts found override rates of 49 to 96 percent across clinical settings (van der Sijs et al., JAMIA, 2006).
...
> Teaching point: measure override rate and dwell time, then break the results down — risk class, queue depth, shift position — because aggregate figures hide exactly this kind of unevenness.

<!--
Speaker notes: Open with the first bullet and mean it: the most common misreading of this slide is that it argues human review is absent, which invites the flat and correct rebuttal that this system has it. The actual argument is narrower and harder to dismiss: human review is the control the entire safety case rests on, and this is the documented failure mode of that specific control; it says nothing about oversight in general. This is the most important failure mode because it defeats the control the whole safety case rests on, and it does so while producing perfect documentary evidence of compliance. Emphasize that the reviewers are not lazy: a system that is right 95 percent of the time trains you to trust it, and that training is rational. Use the two life-sciences citations to close off the 'that is a different industry' objection before it is raised. The alert-override review spans clinical settings directly comparable to this one: a computerized system producing a recommendation a qualified professional is expected to check. The mammography finding is the sharper point and worth lingering on: the effect was largest in the most skilled readers rather than the least skilled, which pre-empts the objection that experienced reviewers would simply not be susceptible to this. IMPORTANT: do not cite specific acceptance or dwell figures for this tool unless you can state their source and are permitted to disclose them. The diagnostic signature is the teachable content and it needs no numbers of your own. If asked whether you measured this, distinguish clearly between what you observed and what you are describing as a documented general pattern. The caseload bullet is a genuine observation from reviewing on-the-ground records during the placement, kept deliberately at the level of shape rather than figures: do not give a case count, a day count, a headcount, or name any reviewer, even if asked directly. The point for this audience is that queue depth and case-age are worth segmenting by at all; no specific number from one site belongs in this room. The turnover bullet is verbal testimony from the team itself, gathered while you were still on the placement. It is not a documented exit-interview finding: attribute it that way if asked, the same way the earlier $9 million figure is attributed to one supervisor rather than presented as an audited number. Do not state a count of resignations beyond 'more than one', do not give a timeframe tighter than your own tenure, and do not name or describe anyone who left. The point is that the workload the team itself was naming as a retention problem is the same variable the diagnostic signature above depends on; whether workload was the sole or officially recorded reason anyone gave for leaving is a separate question this bullet does not answer.
-->
...
- The event itself is not indistinguishable, though: token count against the context limit, and parser success or failure, are both deterministic, loggable signals. Nothing about a truncation event is inherently invisible.
- Cheap to build is not the same as resourced to build, though: the team that would have wired this signal into review is the same one already running an unfunded, reactive maintenance queue with no in-house developer assigned: the fix competes for capacity that, on this project, was not there.

> The reviewer cannot catch it from the output alone. But the event is a replicable, loggable metric: the failure is that nobody surfaces it to review, not that it cannot be surfaced.

<!--
Speaker notes: This is the cleanest example of the unintentional failure class. There is no attacker. There is no negligence. There is no procedural violation. A competent reviewer following the procedure correctly signs off on an assessment built from incomplete input. Ask the audience how their graduates would detect this from the finished report. They cannot: the omission carries no signature in the output. Make the second half explicit, because it is what turns this from a shrug into a teachable fix: this is not an unknowable event. Token-limit and parse-success checks are cheap and already exist in most ingestion pipelines. The gap is that the signal was never wired to the review workflow; the signal itself is not missing. That is what makes this the most directly fixable of the four failure modes in the abstract, but do not let 'cheap' read as 'easy to have actually gotten built here'. The same resourcing gap named on the earlier slide (no allocated developer, a reactive queue already stacked with unaddressed items) is exactly what would have had to absorb this work. If asked who would have built it, say plainly that you do not know who at that function had the spare capacity, because the record shows none was allocated. This failure mode is not a counterexample to the resourcing finding; it is that finding recurring in a different room.
-->
...
- The industry already has an answer to this exact structural question, just not yet for model versions: GMP batch genealogy (the 'where-used' query that identifies every finished-product batch containing a given raw-material lot) is a standard, regulator-expected recall-scoping capability. The gap is that the same discipline is not routinely extended to model versions.
- The scoping problem compounds if the system also adjusts its own future behavior from accumulated corrections, as in the previous failure mode: a defect that shaped what reviewers corrected during the affected window does not stay contained to the records it touched. It can also have shifted the model's behavior on every case that followed, and a cohort query by version and date range alone will not surface that.
- Detection latency raises the stakes further: a defect confirmed a year after introduction, on top of an under-resourced, reactive maintenance function, is not a bounded incident by the time anyone finds it: it is a moving target.

> Most systems can answer 'what happened in this case'. Far fewer can answer 'what else did this affect'.

<!--
Speaker notes: This is the failure mode with the most direct regulatory consequence. Impact assessment and, where warranted, field action both depend on being able to bound the affected population. A traceability capability built for single-case investigation readiness does not automatically give you cohort enumeration by model version and date range. Nobody discovers this until they need it. The batch-genealogy bullet is the one to lean on hardest with this audience: it reframes the fix from 'a new AI capability nobody has built' to 'an existing GMP discipline not yet extended to model versions'. SAP Global Batch Traceability is a concrete, citable example of this capability already deployed at scale for materials. If pushed for evidence this is not AI-specific wishful thinking, commercial ML observability platforms (Arize AI is a verifiable example) already log a model-version field alongside every prediction's timestamp and support exporting exactly that cohort by version and date range. The pattern already exists in ML tooling too, it is simply not yet standard practice inside GMP-adjacent AI deployments specifically. If asked what would address the base version of this problem, say plainly: every record needs a model-version and timestamp tag at processing time, and the traceability system needs a cohort-query capability over that tag. Single-case lookup alone is not enough. That part is ordinary data engineering, though it leaves the compounding case unaddressed: a defect's output has already fed back into the model's own future behavior through the self-adjusting mechanism from the previous slide: a cohort-by-date query no longer bounds that, and nobody in the field has a clean answer for it yet. Keep the year-later scenario hypothetical and structural: the point is that weak detection capacity and a self-adjusting system multiply each other, not that this happened here.
-->
...
- The exposure is second-order and real: a missed chance to catch a wrong decision made by a person, which is different from a wrong decision made by the model itself.
...
- Computer system validation establishes fitness at a point in time; these systems drift continuously. IQ/OQ/PQ and periodic re-validation are retriggered by a change to code or configuration: a system that drifts from accumulated feedback with no code change, as in failure mode 3, simply never trips that trigger.
- Risk-based categorization assumes deterministic input-output behavior. A fixed test suite standing in for exhaustive testing only works if the same input reliably produces the same output; submit the same draft twice and a language model can legitimately return two different scores, and no risk category anticipates that as normal rather than as a defect.
- Annex 11 and Part 11 predate language models and say nothing about acceptance-rate monitoring or context-window limits: the specific blind spots behind failure modes 1 and 2 earlier in this talk.
- Change control and recall scoping assume a bounded, traceable propagation path for a defect, the same batch-genealogy logic behind failure mode 4. A shared model artifact affects every case processed after it, and no requirement mandates the cohort-query capability that would scope that.
- None of these frameworks is wrong. They are necessary and currently insufficient.

> This is not a framework failure so much as a category the framework was never written to cover.

<!--
Speaker notes: Be careful and generous here. This audience teaches these frameworks. The argument is additive rather than dismissive: the existing apparatus is sound for deterministic systems and needs a supplement for probabilistic ones. If you come across as saying validation is broken, you lose the room. Each of the first four bullets is a deliberate callback to one of the four failure modes shown earlier: naming the number out loud is what turns this slide from an abstract framework critique into a demonstration that you can point to the exact concrete gap each framework misses.
-->
...
- A 25-item assessment instrument built for a system that already exists, rather than a design still under review.
...
<!--
Speaker notes: Transition from problem to contribution. The important design choice is that every item asks for observed system behavior or a retrievable artifact, never for an opinion or a self-rating. A reviewer with no machine learning background can execute it, which is the point. Expect the sharpest challenge in the room here, and answer it directly rather than deflect: if no standard existed to identify a competent builder and no procedure existed either, why should anyone trust this instrument, and why did you not simply fix the underlying system yourself? Answer in three parts. First, the instrument's own design does not ask anyone to trust your competence: every item requires an observed system behavior or a retrievable artifact, never a self-rating or an opinion, which is deliberate: its output should not depend on who administers it. Second, it does not claim to be a validated procedure or an approved SOP, but a reviewer aid offered for the same scrutiny this whole talk invites, and the disclaimer says outright that interpretation and sign-off require qualified QA, regulatory, and clinical-safety personnel: it asks to be checked rather than trusted on your say-so. Third, on why the underlying system itself was not fixed: several of the specific gaps identified earlier are ordinary engineering, but ordinary engineering still needs a resourced, in-house developer, and the resourcing slide already established that this layer did not have one. Producing an assessment instrument was within the scope and authority available to a departing product owner. Rewriting another team's production system was not.
-->
...
<!--
Speaker notes: This is the slide a technical audience will engage with most. The default behavior of almost every scorecard is that unanswered questions are silently excluded from the denominator, which means an assessment nobody completed can look like a pass. CMMI is the concrete example if someone asks for one: under the official SCAMPI appraisal method, a goal that lacks sufficient objective evidence is rated 'not rated,' and a process area with insufficient data coverage is excluded from the rating entirely rather than counted as a gap: the industry's own reference maturity model treats missing evidence as neutral rather than as a failure. Inverting that default is the single most important property of the tool.
-->
...
**Synthetic sample data. No findings shown are from a real deployment.**
...
> High line coverage on the scoring engine proves the logic is consistent. It says nothing about whether the rubric's blocking threshold is set correctly.

<!--
Speaker notes: Do not skip this slide, and do not apologize through it. Volunteering the weaknesses is what separates an analysis from an advocacy pitch, and this audience will find these problems anyway. The threshold issue is the one worth genuinely asking their opinion on: if nearly everything blocks, the instrument stops distinguishing a careless deployment from a careful one.
-->
...
- Then I resigned and worked the notice, delivering the transition documentation I had promised.
...
- A concrete example of what such a standard would catch: an interface that requires the user to phrase input in a narrow, undocumented way, and degrades into incoherent output rather than a clear error when they do not: shifting the cognitive burden back onto the person the tool exists to help.

> In this case study, the last control in the chain was a reviewer trained to operate the system, not to audit it.

<!--
Speaker notes: Present this as a finding of the case study rather than a request, and let the faculty draw their own curriculum implications rather than proposing what they should change; if asked directly whether you have a suggestion, the four failure modes and the instrument itself are concrete material you can point to, but lead with the finding rather than the offer. The build-side competency bullet is the one this audience is best placed to evaluate, since defining and assessing competency is their core business. Keep it structural: the finding is an absent standard, and it says nothing about any individual. If asked whether you saw this go wrong in practice, say the standard was absent and stop there. Speculating about whether a particular person met a standard that was never written is neither fair nor arguable, and it will cost you the room instantly. The interface bullet is a generic illustration of what an absent build-side standard misses in practice, not an account of any specific tool or person's work. Do not let it become one if a question tries to make it so. If pressed for a real example, redirect to the general pattern: some interfaces make failure the user's fault by design, and that is exactly what a competency standard should catch before deployment rather than after.
-->

---

## The reviewer question is starting to be tested. The builder question is not.

- Preregistered experiments now directly test reviewer vigilance under AI oversight: a 12-week, double-blind trial had licensed pharmacy reviewers evaluate AI-drafted adverse event summaries under deliberately injected errors, and found the effect on true-error detection was nonmonotone rather than simply 'more errors shown, more caught' (Fang et al., SSRN working paper, 2025).
- A separate randomized experiment found reviewers corrected an overly harsh score less often when it was labeled AI-generated than when the identical score was labeled human-generated: a 22 percent larger fairness gap driven by the label alone (Goulas et al., PNAS Nexus, 2026).
- A parallel line of explainable-AI research is building frameworks for decomposing what a model's stated rationale is actually made of, aimed at giving a reviewer something legible to check against rather than a bare score.
- None of that answers who is competent to build the system a reviewer is checking. The empirical literature testing and qualifying builders for a regulated process is comparatively thin next to the literature testing reviewers.
- That asymmetry is the same one this case study found inside one organization: real, measurable attention to the reviewing side, and very little yet to the building side.

> The two questions this case study opened with are not equally studied. One now has a growing empirical literature. The other does not yet, and that is where I intend to keep working.

<!--
Speaker notes: This slide updates the case study with where the field has moved since the placement ended, so date it clearly as recent, ongoing work rather than something available at the time. Say plainly that neither citation is from life sciences directly: the pharmacovigilance trial is adjacent, drug safety review performed by PharmD reviewers, and the grading study is from education, but the mechanism, a labeled recommendation changing how hard a person pushes back, generalizes to any domain where a human reviews a model's output before it becomes a record. Be exact about what the pharmacovigilance trial found: the relationship between error frequency and detection was not linear. Low-frequency error injection made true-error detection worse before higher frequency made it better. That nuance matters more than the headline, because it means 'show reviewers more errors' is not automatically a fix. The explainability bullet is deliberately generic. Do not claim any specific explanation method would have solved anything on this project; say only that a body of work exists on specifying what an explanation is made of, and that it targets the reviewer side of the problem rather than the builder side. The claim that builder-qualification research is thin is a comparative claim about where research attention has gone, not a claim that no such research exists anywhere. State it that way if pressed. Close by tying this back explicitly to the standard-for-who-builds-it and curriculum-gap slides: this is not a new finding about the case study, it is where you intend to keep working after it.
-->
...
- With 18 of the 25 items able to block on their own, is that the right threshold, or does it make the instrument useless in practice?
- How much should the blocking set flex with the deployment's risk class, if at all?
- Which existing curriculum area is the more natural home for this, validation or quality systems, or does it deserve a module of its own?
- What would convince you that a reviewer assigned to oversee a model is actually competent to do it?
- Explainability research is starting to decompose what a model's rationale is actually made of. Turning that into something a reviewer is trained to use looks like a curriculum question more than a research one: do you agree?
