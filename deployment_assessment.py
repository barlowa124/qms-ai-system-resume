...
<head>
  ...
  <style>
    :root {{
      --bg: #f2f4f7; --card: #ffffff; --ink: #101828; --muted: #475467;
      --accent: #0b4e8f; --accent-soft: #e8f1fb; --line: #e4e7ec;
      --line-strong: #cfd6e0;
      --crit: #b42318; --crit-soft: #fef3f2; --crit-line: #fecdca;
      --major: #b54708; --major-soft: #fffaeb; --major-line: #fedf89;
      --minor: #067647; --minor-soft: #ecfdf3; --minor-line: #a6f4c5;
    }}
    * {{ box-sizing: border-box; }}
    body {{ margin: 0; padding: 40px 20px 56px;
      background:
        radial-gradient(circle at 10% 0%, #e9f4ff 0%, transparent 32%),
        var(--bg);
      color: var(--ink);
      font: 15px/1.55 -apple-system, BlinkMacSystemFont, "Segoe UI", Inter, Roboto, sans-serif;
      -webkit-font-smoothing: antialiased; }}
    .wrap {{ max-width: 1080px; margin: 0 auto; }}
    h1 {{ font-size: 27px; font-weight: 750; letter-spacing: -0.02em;
      margin: 0 0 6px; color: #0a2540; }}
    h2 {{ font-size: 17px; font-weight: 700; letter-spacing: -0.01em;
      margin: 30px 0 12px; padding-bottom: 8px; color: #0a2540;
      border-bottom: 2px solid var(--accent-soft); }}
    h3 {{ font-size: 14.5px; font-weight: 650; margin: 0 0 4px; }}
    ...
    .disclaimer {{ border-left: 4px solid var(--crit); color: var(--muted);
      background: linear-gradient(to right, #fffaf9, var(--card) 30%);
      font-size: 14px; line-height: 1.55; }}
    table {{ width: 100%; border-collapse: collapse; margin-top: 8px;
      font-variant-numeric: tabular-nums; }}
    th, td {{ border: none; border-bottom: 1px solid var(--line); padding: 8px 12px;
      text-align: left; vertical-align: top; font-size: 13px; line-height: 1.45; }}
    th {{ background: #f8fafc; color: #334155; font-size: 11px; font-weight: 700;
      letter-spacing: 0.07em; text-transform: uppercase;
      border-bottom: 2px solid var(--line-strong); }}
    tbody tr:last-child td {{ border-bottom: none; }}
    tbody tr:hover td {{ background: #f8fafd; }}
    code {{ background: #f1f4f9; border: 1px solid var(--line);
      padding: 1px 6px; border-radius: 5px; font-size: 12px; font-weight: 600;
      color: #0a2540; }}
    .item {{ border-top: 1px solid var(--line); padding: 16px 0; }}
    .item:first-of-type {{ border-top: none; }}
    .meta {{ color: var(--muted); font-size: 12.5px; margin: 0 0 8px; line-height: 1.5; }}
    .dq {{ color: var(--crit); }}
    .dq li::marker {{ color: var(--crit); }}
    .na {{ color: var(--muted); font-style: italic; font-size: 13px; }}
    .sev {{ font-size: 11px; font-weight: 650; padding: 3px 9px; border-radius: 999px;
      white-space: nowrap; letter-spacing: 0.02em; border: 1px solid transparent; }}
    .sev-patient_safety_critical {{ background: var(--crit-soft); color: var(--crit);
    ...
    .sev-minor {{ background: var(--minor-soft); color: var(--minor);
      border-color: var(--minor-line); }}
    ul {{ margin: 4px 0 8px 18px; padding: 0; }}
    li {{ margin: 3px 0; }}
    li::marker {{ color: var(--accent); }}
    @media print {{
      body {{ background: #fff; padding: 0; }}
      section, .disclaimer {{ box-shadow: none; break-inside: avoid; }}
      .sev, th {{ print-color-adjust: exact; -webkit-print-color-adjust: exact; }}
    }}
  </style>
</head>
<body>
